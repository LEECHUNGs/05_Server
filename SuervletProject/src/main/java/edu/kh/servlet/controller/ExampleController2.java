...
public class ExampleController2 extends HttpServlet{
	...
	// POST 요청 치리 메서드 오버라이딩
	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		
		// 원래 tomcat9 버전때는 
		// 전달받은 값을 얻어와서 확인 시 한글이 깨지는 문제가 발생!
		// 해결법 : 전달받은 데이터의 문자인코딩을 서버에 맞게 변경
		//req.setCharacterEncoding("UTF-8");
		// -> tomcat10 버전 이상부터 자동으로 인코딩 처리를 해주기에 추가적 필요X
		
		String inputId = req.getParameter("inputName");
